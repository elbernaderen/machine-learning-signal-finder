...
def actual(file):
    ...
    for i in range(p,len(file)):
        ...
        K = len(file)
        if slope < slope_ and (file["volume"][K - 2]> vol_prom * a or file["volume"][K - 1] > vol_prom * a) and (file["rsi"][i-in_] < rsi_ or file["rsi"][i-in_-1] < rsi_):
            vale = 1
        ...
        for t in reversed(range(in_)):
            row = row + [(file["high"][i-t] - file["close"][i-in_]) / file["close"][i-in_]]
        for t in reversed(range(in_)):
            row = row + [(file["low"][i-t] - file["close"][i-in_]) / file["close"][i-in_]]
        new.loc[i] = row
    return new
